...
protocol VideoManagerDelegate: AnyObject {
    func videoManagerDidChangeAvailableDevices(_ audioManager: AudioManager)
    func videoManagerDidChangeAudioDevice(_ audioManager: AudioManager)
}

final class VideoManager {
    internal weak var delegate: VideoManagerDelegate? = nil

    /// user's explicitly preferred device.
    /// nil means "current system default".
    internal var preferredVideoDevice: VideoDeviceType? = nil {
        didSet {
            if self.preferredVideoDevice != oldValue {
    ...
    internal var videoDevice: VideoDeviceType?
    ...
    internal var preferredVideoDeviceIfAvailable: VideoDeviceType? {
        self.preferredVideoDeviceIsAvailable(preferredVideoDevice) ? self.preferredVideoDevice : nil
    }
    ...
    private static var defaultDevice: VideoDeviceType {
        .camera
    }
    ...
    private func configureAudioSessionIfNeeded(suppressDelegateCallbacks: Bool = false) {
        ...
        do {
            ...
            if self.getCurrentAudioDevice() != self.preferredVideoDevice {
                ...
                if videoDevice != newAudioDevice {
                    videoDevice = newAudioDevice
    ...
    private func preferredVideoDeviceIsAvailable(_ preferredVideoDevice: VideoDeviceType?) -> Bool {
        var targetPortTypes: [AVAudioSession.Port]
        var invert = false // whether to check whether targetPortTypes are *not* available
        
        switch preferredVideoDevice {
        case .wired?, .earpiece?:
            targetPortTypes = [.headphones, .headsetMic]
            if case .earpiece = preferredVideoDevice {
    ...
    internal func applyConfiguration() throws {
        ...
        let preferredDeviceToUse = preferredVideoDeviceIfAvailable
    ...
    private func getCurrentAudioDevice() -> VideoDeviceType {
        let defaultDevice: VideoDeviceType = Self.defaultDevice

        guard let firstOutput = self.audioSession.currentRoute.outputs.first else {
            return defaultDevice
        }

        guard let videoDevice = VideoDeviceType(sessionPort: firstOutput.portType) else {
            return defaultDevice
        }
    ...
    private func getAvailableDevices() -> [Device] {
        ...
        var deviceTypes = availableInputs.compactMap { input in
            VideoDeviceType(sessionPort: input.portType)
        }
        ...
        if deviceTypes.contains(VideoDeviceType.speakerphone) {
            deviceTypes.append(VideoDeviceType.earpiece)
        } else {
            deviceTypes.append(VideoDeviceType.speakerphone)
        }

        // When we are using bluetooth as the default route,
        // iOS does not list the bluetooth device on the list of availableInputs
        let outputDevice = availableOutputs.first.flatMap { VideoDeviceType(sessionPort: $0.portType) }
        if let outputDevice {
        ...
        if deviceTypes.contains(VideoDeviceType.wired) {
            deviceTypes = deviceTypes.filter { device in
                device != VideoDeviceType.bluetooth && device != VideoDeviceType.earpiece
            }
        }
